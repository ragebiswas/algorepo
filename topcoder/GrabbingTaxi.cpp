...
public:
	int minTime(vector <int> tXs, vector <int> tYs, int gX, int gY, int walkTime, int taxiTime) {
		int dist = abs(gX) + abs(gY);
		int twalk = dist * walkTime;
		for(int i=0; i<tXs.size(); i++)
		{
			int walkpart = (abs(tXs[i]) + abs(tYs[i])) * walkTime;
			int cabpart = (abs(gX-tXs[i]) + abs(gY-tYs[i])) * taxiTime;
			if (walkpart + cabpart < twalk) 
				twalk = walkpart + cabpart;
		}
		return twalk;
	}
};
