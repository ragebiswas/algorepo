...
public:
	int minChanges(vector <int> groups, int minSize, int maxSize) {
		int deficit = 0, excess = 0, tot = 0;
		for(int i=0; i<groups.size(); i++)
		{
			if(groups[i] >= maxSize) excess += groups[i]-maxSize;
			if(groups[i] < minSize) deficit += minSize-groups[i]; 
			tot += groups[i];
		}
		if(tot < minSize*groups.size() || tot > maxSize*groups.size()) return -1;
